...
Start the day: read, document, describe the model. check every line!?

Problem:
distribition of biomass in model is wider than data. in particular there are too many big cells and too many small cells and way too few mid-size cells.
idea - growth might be wrong (but model is well supported), perhaps because light is too noisy and also Ek is too large. 
idea - division might be wrong - functional form caused Sosik trouble; not well motivated by theoretical underpinnings

Ideas
- use delta as set of constants instead of parameterized functional form (done this wrong, probably; check equations)
- two populatoons?? not a good idea probably
- running median smoother for light
- sample every 3 minutes, 12 minutes, 20 minutes
- use 1h, 6h, 12h, 24h
...
Plot PAR and smoothed PAR
```{r}
plot(time, PAR, pch=20)
PAR_smooth = runmed(PAR, 21)
points(time, PAR_smooth, col='red', pch=1, cex=0.7)
...
parameters {
    ...
    real<lower=0, upper=1.0> delta_i[m]; // AJI: loss; could put a hyperprior on these
}
    ...
    matrix[m,nt_obs] mod_obspos;
    {
        ...
        for (it in 1:nt){ // time-stepping loop
            ...
            for (i in 1:m){ // size-class loop
                ...
                // don't compute delta_i; instead estimate delta_i
                ...
                if (i < j){
                    ...
                    a = gamma * (1.0-delta_i[i]);  // AJI add index
                    w_next[i+1] += a * w_curr[i];
                }
                // fill (j-1)th superdiagonal (division)
                if (i >= j){
                    //A[i+1-j,i] = 2.0*delta_i;
                    a = 2.0*delta_i[i];  // AJI add index; check tricky
                ...
                if (i < j){
                    ...
                    //A[i,i] = (1.0-delta_i);
                    a = (1.0-delta_i[i]); // AJI add index
                    w_next[i] += a * w_curr[i];
                } else {
                    //A[i,i] = (1.0-gamma) * (1.0-delta_i);
                    a = (1.0-gamma) * (1.0-delta_i[i]);  // AJI index
                    w_next[i] += a * w_curr[i];
                }
            }
...
model {
    ...
    for (it in 1:(m-1)) { delta_i ~ uniform(0,1); } // could use beta, could have hyperprior
    
    // fitting observations
    for (it in 1:nt_obs){
        diff = 0.0;
        for (iv in 1:m){
            diff += sqrt((mod_obspos[iv,it] - obs[iv,it])^2.0);
        }
        diff = diff/sigma;
...
```{r}
mcmc <- sampling(mod,data=data,open_progress=FALSE,control=list(adapt_delta=0.95))
```


Check chains to be sure they are mixing well. Rhat values.

```{r}
summary(mcmc,pars=c('b','delta_max','gamma_max','E_star','sigma', 'delta_i'))$summary
```

```{r}
stan_plot(mcmc, pars="delta_i")
```
